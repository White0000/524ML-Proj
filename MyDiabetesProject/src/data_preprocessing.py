...
class ModelEvaluator:
    def __init__(self, model_path=None, calibration=False, folds=5):
    ...
    def run(self, save_report=False, report_path="evaluation_report.json"):
        ...
        if not model:
            return
        if self.calibration and hasattr(model, "predict_proba"):
            model = self.calibrate_model(model, x_train, y_train)
        train_preds = model.predict(x_train)
        test_preds = model.predict(x_test)
        results = {
            "train": self.evaluate(x_train, y_train, train_preds, model),
            "test": self.evaluate(x_test, y_test, test_preds, model)
        }
        if save_report:
    ...
    def load_data(self):
        try:
            ...
            if not csv_path.exists():
                logger.error(str(csv_path) + " not found")
    ...
    def calibrate_model(self, model, x_train, y_train):
        c = CalibratedClassifierCV(base_estimator=model, cv=self.folds)
...
if __name__ == "__main__":
    ModelEvaluator(calibration=True, folds=3).run(save_report=True)
